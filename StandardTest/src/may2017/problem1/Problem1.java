...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		List<String> once = Stream
				.concat(list1.stream().filter(e -> !list2.contains(e)), list2.stream().filter(e -> !list1.contains(e)))
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		return list.stream().filter(c -> c.getCity().equals("Fairfield")).map(Customer::getName).sorted()
				.collect(Collectors.toList());
	}
