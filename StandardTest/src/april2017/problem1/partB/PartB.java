package april2017.problem1.partB;
...
public class PartB {
	...
	Function<CheckoutRecord, List<CheckoutRecordEntry>> fun = record -> record.getCheckoutEntries();

	// provide a method reference and the type of method reference
	// TYPE:
	Function<CheckoutRecord, List<CheckoutRecordEntry>> mr = CheckoutRecord::getCheckoutEntries;

	// provide an inner class that behaves the same way as the lambda
	// class My--- implements --- { }
	class MyClass implements Function<CheckoutRecord, List<CheckoutRecordEntry>> {
	...
	public void evaluator() {
		System.out.println(fun.apply(record));
		System.out.println(mr.apply(record));
	...
	public static void main(String[] args) {
		...
		List<CheckoutRecordEntry> allEntries = new ArrayList<CheckoutRecordEntry>() {
			{
				add(new CheckoutRecordEntry(allBooks.get(0).getCopies().get(0), LocalDate.of(2011, 12, 1),
						LocalDate.of(2011, 12, 22)));
				add(new CheckoutRecordEntry(allBooks.get(0).getCopies().get(1), LocalDate.of(2014, 6, 22),
						LocalDate.of(2014, 7, 13)));
				add(new CheckoutRecordEntry(allBooks.get(1).getCopies().get(0), LocalDate.of(2015, 12, 2),
						LocalDate.of(2015, 12, 23)));
				add(new CheckoutRecordEntry(allBooks.get(2).getCopies().get(0), LocalDate.of(2015, 6, 24),
